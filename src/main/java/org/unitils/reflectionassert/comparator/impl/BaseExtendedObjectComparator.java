...
/**
 * Comparator for objects. it will check if the field need to be compare!
 *
 * @author Tim Ducheyne
 * @author Filip Neven
 * @author ruiyong.hry
 */
public abstract class BaseExtendedObjectComparator implements Comparator {
    ...
    protected abstract boolean fieldNeedCompare(String className, String fieldName);

    protected TransferValueWrapper transformValue(Object left, Object right) {
        TransferValueWrapper transferValueWrapper = new TransferValueWrapper(false);
        transferValueWrapper.setTleft(left);
        transferValueWrapper.setTright(right);
        return transferValueWrapper;
    }

    public static class TransferValueWrapper {
        private Object tleft;
        private Object tright;
        private boolean isTransfromed;

        public TransferValueWrapper(boolean isTransfromed) {
            this.isTransfromed = isTransfromed;
        }


        public boolean isTransfromed() {
            return isTransfromed;
        }

        public void setTransfromed(boolean transfromed) {
            isTransfromed = transfromed;
        }

        public Object getTleft() {
            return tleft;
        }

        public void setTleft(Object tleft) {
            this.tleft = tleft;
        }

        public Object getTright() {
            return tright;
        }

        public void setTright(Object tright) {
            this.tright = tright;
        }
    }
        ...
        for (Field field : fields) {
            try {
                if (PathTrace.isHandleNode(reflectionComparator.getCompareConfig())) {
                    PathTrace.enterNode(field.getName());
                }
                ...
                try {
                    TransferValueWrapper transferValueWrapper = transformValue(field.get(left), field.get(right));
                    // recursively check the value of the fields
                    //判断是否需要对比
                    if (!fieldNeedCompare(clazz.getName(), field.getName())) {
                        continue;
                    }

                    Difference innerDifference = reflectionComparator.getDifference(transferValueWrapper.getTleft(), transferValueWrapper.getTright(), onlyFirstDifference);
                ...
                if (PathTrace.isHandleNode(reflectionComparator.getCompareConfig())) {
                    PathTrace.exitNode(field.getName());
                }
            }
        }
