...
public class DefaultDifferenceView implements DifferenceView {
    ...
    protected String formatDifference(Difference difference, String fieldName) {
        return formatValues(fieldName, difference.getFieldClass(), difference.getLeftValue(), difference.getRightValue());
    }
    ...
    protected String formatDifference(CollectionDifference collectionDifference, String fieldName) {
        ...
        for (Map.Entry<Integer, Difference> elementDifferences : collectionDifference.getElementDifferences().entrySet()) {
            ...
            Difference difference = elementDifferences.getValue();
            result.append(difference.accept(differenceFormatterVisitor, innerFieldName));
        }
        ...
        for (Integer leftIndex : collectionDifference.getLeftMissingIndexes()) {
            ...
            result.append(formatValues(innerFieldName, collectionDifference.getFieldClass(), leftList.get(leftIndex), DefaultDifferenceReport.MatchType.NO_MATCH));
        }
        for (Integer rightIndex : collectionDifference.getRightMissingIndexes()) {
            ...
            result.append(formatValues(innerFieldName, collectionDifference.getFieldClass(), DefaultDifferenceReport.MatchType.NO_MATCH, rightList.get(rightIndex)));
        }
        return result.toString();
    }
     ...
     * @return The string representation, not null
     */
    protected String formatDifference(MapDifference mapDifference, String fieldName, Class<?> fieldClass) {
        StringBuilder result = new StringBuilder();
        ...
        for (Object leftKey : mapDifference.getLeftMissingKeys()) {
            ...
            result.append(formatValues(innerFieldName, fieldClass, leftMap.get(leftKey), ""));
        }
...
//            result.append(formatValues(innerFieldName, fieldClass, rightMap.get(rightKey), ""));
            result.append(formatValues(innerFieldName, fieldClass, "", rightMap.get(rightKey)));
        }
        return result.toString();
    }
    ...
    protected String formatDifference(UnorderedCollectionDifference unorderedCollectionDifference, String fieldName) {
        ...
        Class fieldClass = unorderedCollectionDifference.getFieldClass();
        ...
        for (Map.Entry<Integer, Integer> bestMatchingIndex : bestMatchingIndexes.entrySet()) {
            ...
            if (leftIndex == -1) {
                ...
                result.append(formatValues(innerFieldName, fieldClass, DefaultDifferenceReport.MatchType.NO_MATCH, unorderedCollectionDifference.getRightList().get(rightIndex)));
                continue;
            }
            if (rightIndex == -1) {
                String innerFieldName = createFieldName(fieldName, "[" + leftIndex + ",x]", false);
                result.append(formatValues(innerFieldName, fieldClass, unorderedCollectionDifference.getLeftList().get(leftIndex), DefaultDifferenceReport.MatchType.NO_MATCH));
                continue;
            }
     ...
     * @return The string representation, not null
     */
    protected String formatValues(String fieldName, Class<?> fieldClass, Object leftValue, Object rightValue) {
        String leftValueFormatted = formatObject(leftValue);
        String rightValueFormatted = formatObject(rightValue);
        String valuesFormattedOnOneLine = formatValuesOnOneLine(fieldName, fieldClass, leftValueFormatted, rightValueFormatted);
        if (valuesFormattedOnOneLine.length() < DefaultDifferenceReport.MAX_LINE_SIZE) {
            return valuesFormattedOnOneLine;
        } else {
            return formatValuesOnMultipleLines(fieldName, fieldClass, leftValueFormatted, rightValueFormatted);
        }
    }

    protected String formatValuesOnOneLine(String fieldName, Class<?> fieldClass, String leftValueFormatted, String rightValueFormatted) {
        StringBuilder result = new StringBuilder();
        if (fieldName != null) {
            result.append(fieldName).append("(" + getClassName(fieldClass) + ")").append(": ");
        }
    ...
    protected String formatValuesOnMultipleLines(String fieldName, Class<?> fieldClass, String leftValueFormatted, String rightValueFormatted) {
        StringBuilder result = new StringBuilder();
        if (fieldName != null) {
            result.append(fieldName).append("(" + getClassName(fieldClass) + ")").append(":\n");
        }
